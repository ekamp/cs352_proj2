...
public class p2padmin implements Runnable {
	...
	public static void main(String args[]) throws Exception {
		...
		while ((line = userdata.readLine()) != null) {
			...
			int fileindex = line.indexOf('/', space+1);
			space = line.indexOf(' ', fileindex);
			String filename = line.substring(fileindex, space);
			String localfile = line.substring(space+1);
			...
			if (command.equals("put")) {
				putrequest = "PUT " + filename + " HTTP/1.1";
			}
			
            try {
                toServer.writeBytes(putrequest + '\n');
